...
# Z = X**2 + 2*Y**2 + np.exp(X**2 + Y**2) - X + 2*Y
#
...
# y_line = x_line + 0.5
# plt.plot(x_line, y_line, 'r--', label='Ограничение: y = x + 0.5')
# # y_line = x_line + 0.2 + 0.1 * f(np.array([x_line, x_line + 0.2 + 0.1 * f(np.array([x_line, x_line]))]))
# # plt.plot(x_line, y_line, 'r--', label='Ограничение: $y = x + 0.2 + 0.1z$')
#
# # Точки минимума
# plt.plot(x0[0], x0[1], 'bo', label='x0')
# plt.plot(minimum_penalty[0], minimum_penalty[1], 'ro', label='Минимум')
#
# plt.xlabel("$x_1$")
# plt.ylabel("$x_2$")
# plt.legend()
